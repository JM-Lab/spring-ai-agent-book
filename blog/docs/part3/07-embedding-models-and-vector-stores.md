...
- `similarityThreshold`: 결과에 들어갈 최소 점수(0.0~1.0)이며 기본값 0.0은 사실상 점수로 거르지 않습니다. 백분율이 아니라 임베딩 모델과 거리 계산 방식에 따라 분포가 달라지는 상대 점수이므로, 처음에는 0.0으로 결과를 모두 보고 무관한 문서가 걸러지기 시작하는 점수를 실제 데이터로 찾습니다.

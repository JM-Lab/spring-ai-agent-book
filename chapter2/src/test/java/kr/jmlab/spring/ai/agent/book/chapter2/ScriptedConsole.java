...
/**
 * CLI Step 러너에 사용자 입력을 흉내 내어 공급하는 테스트 헬퍼("타이핑 시뮬레이터").
 *
 ...
 */
final class ScriptedConsole implements AutoCloseable {

    static final String DELAY_PROPERTY = "chapter2.cli.input-delay-ms";

    /** 입력 에코 여부(기본 켬). {@code -Dchapter2.cli.echo=false} 로 끈다. */
    static final String ECHO_PROPERTY = "chapter2.cli.echo";
    ...
    private ScriptedConsole(List<String> scriptLines, long delayBetweenLinesMillis) {
        ...
        this.writerThread = new Thread(
                () -> supplyLines(scriptLines, delayBetweenLinesMillis),
                "scripted-console-typist");
     ...
     */
    static ScriptedConsole withDefaultDelay(String... scriptLines) {
        long delayBetweenLinesMillis = Long.getLong(DELAY_PROPERTY, DEFAULT_DELAY_MILLIS);
        return new ScriptedConsole(List.of(scriptLines), delayBetweenLinesMillis);
    }
    ...
    private void supplyLines(List<String> scriptLines, long delayBetweenLinesMillis) {
        try {
            for (String scriptLine : scriptLines) {
                writeLine(scriptLine, true);

...
/**
 * IT 실행 동안 {@code System.out} 을 콘솔과 캡처 파일({@code target/ch2-capture/<step>.txt})로
...
final class CapturingConsole implements AutoCloseable {
    ...
    private static final Path CAPTURE_DIR = Path.of("target", "ch2-capture");
    ...
    /** 지정한 스텝 이름으로 캡처를 시작한다({@code target/ch2-capture/<step>.txt}). */
